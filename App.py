...
def compute_ghana_payroll_taxes(basic_salary, allowances, bonus, overtime, additional_deductions, tier3_rate):
    ...
    """
    # 1. Statutory Pension Deductions (SSNIT Tier 1 & 2 Employee Share)
    mandatory_ssnit = basic_salary * 0.055
    
    # 2. Voluntary Tier 3 Assessment with statutory 16.5% tax-free ceiling cap
    ...
    # 4. Assess Total Chargeable Income (Reliefs and SSNIT deducted first)
    ...
    # 5. Progressive Tax Band Calculation Logic
    gra_tax_bands = [
    ...
    # Top marginal bracket for remaining income exceeding the bands
    if remaining_taxable > 0:
...
_, edu_center, _ = st.columns([0.5, 5, 0.5])
with edu_center:
