...
def generate_primary_pdf_report(primary_details, primary_final_net_take_home, primary_tbill):
    ...
    primary_pdf.cell(200, 10, "Official Payroll Computation Report - Primary Salary Layout", ln=1, align="C")
    primary_pdf.cell(200, 10, "-------------------------------------------------------------------------", ln=2, align="C")
    
    primary_pdf.cell(200, 10, f"Total Gross Earnings: GHS {primary_details['primary_gross']:.2f}", ln=3)
    primary_pdf.cell(200, 10, f"Mandatory SSNIT Contribution (5.5%): GHS {primary_details['primary_ssnit']:.2f}", ln=4)
    primary_pdf.cell(200, 10, f"Pension Plan Savings: GHS {primary_details['primary_tier3']:.2f}", ln=5)
    primary_pdf.cell(200, 10, f"Taxable Chargeable Income: GHS {primary_details['primary_taxable']:.2f}", ln=6)
    primary_pdf.cell(200, 10, f"Income Tax Paid to GRA (PAYE): GHS {primary_details['primary_tax']:.2f}", ln=7)
    primary_pdf.cell(200, 10, f"Total Deductions Applied: GHS {primary_details['primary_total_deductions']:.2f}", ln=8)
    primary_pdf.cell(200, 10, f"Effective Income Tax Rate: {primary_details['primary_effective_tax_rate']:.2f}%", ln=9)
    primary_pdf.cell(200, 10, f"Short Term Treasury Bill Target: GHS {primary_tbill:.2f}", ln=10)
    primary_pdf.cell(200, 10, f"Final Net Take Home: GHS {primary_final_net_take_home:.2f}", ln=11)
    
    return bytes(primary_pdf.output(dest="S"))
...
def generate_alternative_pdf_report(alternative_details, alternative_final_net_take_home, alternative_tbill):
    ...
    alternative_pdf.cell(200, 10, "Official Payroll Computation Report - Alternative Salary Layout", ln=1, align="C")
    alternative_pdf.cell(200, 10, "-------------------------------------------------------------------------", ln=2, align="C")
    
    alternative_pdf.cell(200, 10, f"Total Gross Earnings: GHS {alternative_details['alternative_gross']:.2f}", ln=3)
    alternative_pdf.cell(200, 10, f"Mandatory SSNIT Contribution (5.5%): GHS {alternative_details['alternative_ssnit']:.2f}", ln=4)
    alternative_pdf.cell(200, 10, f"Pension Plan Savings: GHS {alternative_details['alternative_tier3']:.2f}", ln=5)
    alternative_pdf.cell(200, 10, f"Taxable Chargeable Income: GHS {alternative_details['alternative_taxable']:.2f}", ln=6)
    alternative_pdf.cell(200, 10, f"Income Tax Paid to GRA (PAYE): GHS {alternative_details['alternative_tax']:.2f}", ln=7)
    alternative_pdf.cell(200, 10, f"Total Deductions Applied: GHS {alternative_details['alternative_total_deductions']:.2f}", ln=8)
    alternative_pdf.cell(200, 10, f"Effective Income Tax Rate: {alternative_details['alternative_effective_tax_rate']:.2f}%", ln=9)
    alternative_pdf.cell(200, 10, f"Short Term Treasury Bill Target: GHS {alternative_tbill:.2f}", ln=10)
    alternative_pdf.cell(200, 10, f"Final Net Take Home: GHS {alternative_final_net_take_home:.2f}", ln=11)
    
    return bytes(alternative_pdf.output(dest="S"))
